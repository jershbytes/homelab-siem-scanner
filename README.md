# Homelab SIEM & Security Scanner Orchestrator
...
## Project Intent
To deliver a streamlined, lightweight framework that provisions network isolation barriers, deploys essential threat-scanning dependencies, and bootstraps a production-ready **Wazuh & ELK Stack** ecosystem on a single node.
...
* **Core Monitoring Layer (Wazuh & ELK Engine)**: Standardizes the ingestion of host security telemetry and exposes a comprehensive dashboard for threat analytics.
* **Integrated Scanner Suite**: Pulls down and hooks standard diagnostic utilities into system log paths:
  ...
  * `jq` & `openssl`: Structured data manipulation and cryptographic operations.
  * `wireguard-tools`: Hooks prepared for secure network transport tunnels.
